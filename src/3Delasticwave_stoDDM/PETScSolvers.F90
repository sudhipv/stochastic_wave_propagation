...
SUBROUTINE PETSc_stonncpcgm(pid,np,nb,npg,nbg,nr,nc,nbgc,Ui,Ub,Ub_g,&
    npcein,npceout,ncijk,ijk,cijk,mallocsCals,nVec,maxiter,tol,T, tcount, deltaT, beta_NB, gamma_NB,outputFlag)
...
!! Fortran-Variables Declaration
    integer :: pid, i, maxiter, nVec, mallocsCals
    integer :: np, ni, nb, nbg, npg
    integer :: nip, nbp, nrp, ncp, nirp, nbgp, nbgcp
    integer :: nc, nr, nbgc, nir,npceout, ncijk, npcein !casep,
    integer :: One, Zero, outputFlag

    integer, dimension((np-nb)*nVec*npceout) :: tempi                !! Temp arrays
    integer, dimension((np-nc)*nVec*npceout) :: tempir               !! require for VecGetVales
    integer, dimension(nc*nVec*npceout)      :: tempc
    integer, dimension(nr*nVec*npceout)      :: tempr
    integer, dimension(nb*nVec*npceout)      :: tempb
    integer, dimension(nbg*nVec*npceout)     :: tempbg
    integer, dimension(nbgc*nVec*npceout)    :: tempcg
    integer, dimension(ncijk,3)              :: ijk


    integer, allocatable, dimension(:) :: tb            !! Array of indices for boundary nodes below interior nodes

    double precision, dimension(nbg*nVec*npceout)     :: Ub_g, rb_g, Qb_g
    double precision, dimension(nbg*nVec*npceout)     :: Pb_g, Zb_g, RZb
    double precision, dimension(nbgc*nVec*npceout)    :: Dc, Zc, DcBc
    double precision, dimension(ncijk)                :: cijk
    double precision, dimension(nr*nVec*npceout)      :: Vri, FrS
    double precision, dimension((np-nc)*nVec*npceout) :: ybc
...
!!!! Vectors used for assembling global solition
    ...
    character(len=255) :: label, extension, filename, str1,str5
...
!! PETSc-Initialize
    ...
    nip  = (ni*nVec*npceout)    !! sto-interior nodes
    nbp  = (nb*nVec*npceout)    !! sto-local-boundary nodes
    ncp  = (nc*nVec*npceout)    !! sto-local-corner nodes
    nrp  = (nr*nVec*npceout)    !! sto-local-remaining nodes
    nirp = (nir*nVec*npceout)   !! sto-interior+remaining nodes
    nbgp = (nbg*nVec*npceout)   !! sto-global-boundary nodes
    nbgcp= (nbgc*nVec*npceout)  !! sto-global-corner nodes


    allocate(tb(nb*nVec*npceout), u0i(ni*nVec*npceout), u0b(nb*nVec*npceout), v0i(ni*nVec*npceout), v0b(nb*nVec*npceout), a0i(ni*nVec*npceout), a0b(nb*nVec*npceout))
    allocate(U_g(npg*nVec),U_gi(npg*nVec))
    allocate(Af(np*nVec,npceout),Afb(nbg*nVec,npceout), var_g(npg*nVec))
...
!! Assemble only Subdomain-Level Deterministic matrices to calculate Mallocs
    ...
    IF (mallocsCals .eq. 1) then
        ...
        call GetMallocsShort(pid,np,nb,nc,nr,npceout,nip,nbp,ncp,nrp,nirp,nVec,ierr)
...
!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call GetStoMatSeqTwoLevel(pid,PAmat,PAii,PAgg,PAgi,PAri,PArr,PAcc,PAci,PAcr,PMmat,PMii,PMgg,PMgi,PMri,PMrr,PMcc,PMci,PMcr,&
                           PCmat,PCii,PCgg,PCgi,PCri,PCrr,PCcc,PCci,PCcr,np,nb,nc,nr,npcein,npceout,ncijk,ijk,cijk,ni,nip,nbp,&
...
! Directly taking the saved initial condition - deterministic so only mean term necessary.
    call GetStoInitialCondSubdomain(pid,ni,nb,u0i,u0b,v0i,v0b,a0i,a0b,nVec, ierr)
...
DO nbcount = 1,tcount
    ...
    call GetMassDampMultiplier(pid,nbcount,nip,nbp,deltaT,beta_NB,gamma_NB, u0i,u0b,v0i,v0b,a0i,a0b,tempi,tempb, PMMuvi, PMMuvb, PCMuvi, PCMuvb,nVec,ierr)
    ...
    call GetForceVecSeq(pid, nbcount, PFi, PFg, ni, nb,nip,nbp, nVec, ierr)


    call GetTransientForce(pid,nbcount,nip,nbp,PMii,PMgg,PMgi,PCii,PCgg,PCgi, PMMuvi, PMMuvb, PCMuvi, PCMuvb, Mni, Mnb, Cni, Cnb, PFi, PFg,nVec, ierr)
...
! Sending one instead of npceout to replicate deterministic case
    call GetRs(pid,nb,nbg,npceout,nVec,RsMat)                   !! Sparse Rs Operator
...
! !!-----------------------------------------------------------------------------------------------
    ...
    call GetDs(pid,nb,npceout,nVec,DsMat)                       !! Sparse Ds Operator

    call MatMult(DsMat,SolVecb,PetVecb,ierr)

    !! SUB-STEP 5 & 6 : Compute
    call getRr(pid, nb, nr,npceout, nVec, RrMat)                !! Sparse RrS Operator
    ...
    call getRc(pid, nb, nc,npceout, nVec, RcMat)                !! Sparse RcS Operator
...
!!!------------------- PMV:Sec1: Scr*v1 = [Acr - (Aci *inv(Aii)*Air)]*v1- -------------------!!!
 ...
 !!!! Part of Eq. 3.28 Ajits thesis !!!!!!!!!!!!!
    ...
    !! SUB-STEP 10 : Gather
    call GetBc(pid, nc, nbgc, npceout,nVec, BcMat)              !! Sparse RcS Operator
...
!!-----------------------------------------------------------------------------------------------
    ...
    call solve_coarsepcgm(pid,nirp,ncp,nrp,nip,nbgcp,BcMat,kspAm,kspAi,kspAc,pcAm,pcAi,pcAc, &
                        ...
                        PAci,PAri,PAcr,PAcc,Dc,Zc,tolc,stages(5))
...
!!!---------------------------------------------------------------------------------------------
    ...
    if (outputFlag .eq. 1) then
        ...
        call create_Af_pckfddm3D(Zero,npceout,np,nb,Ui,Ub,nVec,Af)      !!!! Function to create full solution but Ui is only for a particular subdomain
        if (pid .eq. 0) then
          call create_Afb_pckfddm3D(Zero,npceout,nbg,Ub_g,nVec,Afb)      !!!! Temporary storing of boundary solution needed for final creation after reduce
        end if
       ...
       do i = 2,npceout
          ...
          if (pid .eq. 0) then
             ...
             !!!***** Higher Order PCE Coefficients *****!!!
                 ...
                 if(i .lt. 12) then
                     call int2str(extension,nbcount,4)
                     call int2str(str5,i,2)
                     label= '../../data/vtkOutputs/pce_' // trim(str5) // '_' // trim(extension) // '.dat'
       ...
       if (pid .eq. 0) then
          ...
          call int2str(extension,nbcount,4)
          var_g = sqrt(var_g)
          label= '../../data/vtkOutputs/sd_' // trim(extension) // '.dat'
          open(unit=3,file=label,status='replace')
          write(unit=3,fmt='(ES17.8E3)') var_g
          close(3)
...
if (pid .eq. 0) then
    call cpu_time(time222)
    if (pid .eq. 0) print*, '---------------------------------------------------------'
    print*, 'CPU-Time by solver',time222-time111,'seconds'
    if (pid .eq. 0) print*, '---------------------------------------------------------'
end if
...
SUBROUTINE solve_coarsepcgm(pid,nirp,ncp,nrp,nip,nbgcp,BcMat,kspAm,kspAi,kspAc,pcAm,pcAi,pcAc, &
                        tempir, tempr, tempcg, PetVecc2, PetVecc3, PetVeccg, ybc, Vri,DcBc, &
                        PetVeci,SolVeci,PetVecir,SolVecir,PetVecc,Solvecc,PetVecr,SolVecr, &
                        PAci,PAri,PAcr,PAcc,Dc,Zc,tolc,stageC)
