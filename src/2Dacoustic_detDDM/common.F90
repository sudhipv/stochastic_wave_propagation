...
!!!! Adding function to read newmark beta parameters
...
!!!! Sudhi
subroutine readNB_Para(T,deltaT,alpha,beta,extension)

    double precision :: T, deltaT, alpha, beta
    character(len=255) :: filename,extension

    filename = '../../external/dolfin/src/NBparam' // trim(extension) // '.dat'
    open(unit=1,file=filename,status='old')
    read(1,*) T, deltaT, alpha, beta
    close(1)

end subroutine readNB_Para
