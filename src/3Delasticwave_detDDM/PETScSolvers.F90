...
SUBROUTINE PETSc_detnncpcgm(pid,np,nb,npg,nbg,nr,nc,nbgc,Ui,Ub,Ub_g,nVec,maxiter,tol,T, tcount, deltaT, beta_NB, gamma_NB,outputFlag)
...
!! Fortran-Variables Declaration
    ...
    integer, dimension((np-nb)*nVec) :: tempi                !! Temp arrays
    integer, dimension((np-nc)*nVec) :: tempir               !! require for VecGetVales
    integer, dimension(nc*nVec)      :: tempc
    integer, dimension(nr*nVec)      :: tempr
    integer, dimension(nb*nVec)      :: tempb
    integer, dimension(nbg*nVec)     :: tempbg
    integer, dimension(nbgc*nVec)    :: tempcg

    integer, allocatable, dimension(:) :: tb            !! Array of indices for boundary nodes below interior nodes

    double precision, dimension(nbg*nVec)     :: Ub_g, rb_g, Qb_g
    double precision, dimension(nbg*nVec)     :: Pb_g, Zb_g, RZb
    double precision, dimension(nbgc*nVec)    :: Dc, Zc, DcBc
    double precision, dimension(nr*nVec)      :: Vri, FrS
    double precision, dimension((np-nc)*nVec) :: ybc
    double precision, dimension(nb*nVec)      :: Ub
    double precision, dimension((np-nb)*nVec) :: Ui

!!!! Vectors used for assembling global solition
    double precision, dimension(npg*nVec)    :: U_g
    double precision, dimension(npg*nVec)    :: U_gi
    ...
    character(len=255) :: label, extension, filename, str1
...
!!---------------------------------------------------------------------------------------------
integer :: cc111,cc222,ccr333
double precision :: time111
double precision :: time222
call system_clock(count_rate=ccr333)
...
!! PETSc-Initialize
    ...
    allocate(tb(nb*nVec), u0i(ni*nVec), u0b(nb*nVec), v0i(ni*nVec), v0b(nb*nVec), a0i(ni*nVec), a0b(nb*nVec))
    allocate(Af(np*nVec,1),Afb(nbg*nVec,1))
...
!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! call GetInitialCond(pid, ni,np,u0i,u0b,v0i,v0b,a0i,a0b,nVec,ierr)


    call GetVecSeqTemp2(nir*nVec,PetVecir,Solvecir,tempir,ierr)
    call GetVecSeqTemp2(ni*nVec,PetVeci,Solveci,tempi,ierr)
    call GetVecSeqTemp2(nb*nVec,PetVecb,Solvecb,tempb,ierr)
    call GetVecSeqTemp2(nr*nVec,PetVecr,Solvecr,tempr,ierr)
    call GetVecSeqTemp2(nc*nVec,PetVecc,Solvecc,tempc,ierr)


    call GetVecSeqDummy(nbgc*nVec,PetVeccg,tempcg,ierr)
    call GetVecSeqDummy(nbg*nVec,PetVecbg,tempbg,ierr)

    call GetVecSeqTemp1(nc*nVec,PetVecc2,ierr)
    call GetVecSeqTemp1(nc*nVec,PetVecc3,ierr)
    call GetVecSeqTemp1(nb*nVec,PetVecb,ierr)
    call GetVecSeqTemp1(ni*nVec,PetVeci,ierr)
    call GetVecSeqTemp1(nb*nVec,PPb,ierr)


    call GetInitialCondSubdomain(pid,ni,nb,u0i,u0b,v0i,v0b,a0i,a0b,nVec, ierr)
...
DO nbcount = 1,tcount
    ...
    call GetMassDampMultiplier(pid,nbcount,ni,nb,deltaT,beta_NB,gamma_NB, u0i,u0b,v0i,v0b,a0i,a0b,tempi,tempb, PMMuvi, PMMuvb, PCMuvi, PCMuvb,nVec,ierr)

    ! print*, 'process id', pid
    ! print*, u0b
    ! call VecView(PMMuvi,PETSC_VIEWER_STDOUT_SELF, ierr);

    call GetForceVecSeq(pid, nbcount, tempi, tempb, PFi, PFg, ni, nb, nVec, ierr)


    call GetTransientForce(pid,nbcount,ni,nb,PMii,PMgg,PMgi,PCii,PCgg,PCgi, PMMuvi, PMMuvb, PCMuvi, PCMuvb, Mni, Mnb, Cni, Cnb, PFi, PFg,nVec, ierr)
...
!!! ALL the restriction and its transpose operators just take nVec and original nodes..other functions take node * nVec.
!!!---------------------------------------------------------------------------------------------
...
! Sending one instead of npceout to replicate deterministic case
    call GetRs(pid,nb,nbg,One, nVec,RsMat)                   !! Sparse Rs Operator
    ...
    call VecGetValues(PetVecbg, nbg*nVec, tempbg, RZb, ierr)   !! PETSc to Fortran Vec

    !! STEP 5 : Gather                                     !! Use PETSc_Reduce in future
    CALL MPI_ALLREDUCE(RZb,rb_g,nbg*nVec,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
...
! !!-----------------------------------------------------------------------------------------------
    ! SUB-STEP 3 : Scatter
    call SetVecSeq(nbg*nVec,rb_g,PetVecbg,ierr)                !! Fortran to PETSc Vec
    call MatMult(RsMat,PetVecbg,SolVecb,ierr)              !! Sparse PETSc Mat*Vec

    !! SUB-STEP 4 : Weighting
    call GetDs(pid,nb,One,nVec,DsMat)                       !! Sparse Ds Operator

    call MatMult(DsMat,SolVecb,PetVecb,ierr)

    !! SUB-STEP 5 & 6 : Compute
    call getRr(pid, nb, nr,One, nVec, RrMat)                !! Sparse RrS Operator
    ...
    call VecGetValues(PetVecr, nr*nVec, tempr, FrS, ierr)      !! Convert FrS to PETScVec-
                                                           !!-For future modification
    call getRc(pid, nb, nc,One, nVec, RcMat)                !! Sparse RcS Operator
...
! !!-------------------------This is the solving stage v1 = s_rr \f_r, but is implemented in a different way than algorithm-------------------------------------------
    ...
    call SetVecSeq(nir*nVec,ybc,PetVecir,ierr)
    call PETSc_KSP(kspAm,pcAm,PAmat,PetVecir,SolVecir,ierr)
    call VecGetValues(SolVecir, nir*nVec, tempir, ybc, ierr)   !! Re-using ybc : Check for error

    Vri = ybc((ni*nVec+1):nir*nVec)                                !! Re-using Vr1 : Check for error
...
!!!------------------- PMV:Sec1: Scr*v1 = [Acr - (Aci *inv(Aii)*Air)]*v1- -------------------!!!
 ...
 !!!! Part of Eq. 3.28 Ajits thesis !!!!!!!!!!!!!
    ...
    ! SUB-STEP 8 : Compute
    call SetVecSeq(nr*nVec,Vri,PetVecr,ierr)
    ...
    !! SUB-STEP 10 : Gather
    call GetBc(pid, nc, nbgc, One,nVec, BcMat)              !! Sparse RcS Operator
    call MatMultTranspose(BcMat,PetVecc,PetVeccg,ierr)
    call VecGetValues(PetVeccg, nbgc*nVec, tempcg, DcBc, ierr)
    CALL MPI_ALLREDUCE(DcBc,Dc,nbgc*nVec,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
...
!!-----------------------------------------------------------------------------------------------
    ...
    call solve_coarsepcgm(pid,nir*nVec,nc*nVec,nr*nVec,ni*nVec,nbgc*nVec,BcMat,kspAm,kspAi,kspAc,pcAm,pcAi,pcAc, &
...
!!-----------------------------------------------------------------------------------------------
    ...
    !!-----------------------------------------------------------------------------------------------
    !! SUB-STEP 14 : Scatter
    CALL MPI_BCAST(Zc,nbgc*nVec,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)

    call SetVecSeq(nbgc*nVec,Zc,PetVeccg,ierr)
...
!!!------------------- PMV:Sec1: Src*v1 = [Arc - (Ari *inv(Aii)*Aic)]*v1- -------------------!!!
    ...
    call VecGetValues(PetVecr, nr*nVec, tempr, Vri, ierr)
...
!!-----------------------------------------------------------------------------------------------
    !! SUB-STEP 17 : Solve
    ybc(:) = 0.0d0
    ybc((ni*nVec+1):nir*nVec) = Vri

    call SetVecSeq(nir*nVec,ybc,PetVecir,ierr)
    call KSPSolve(kspAm,PetVecir,SolVecir,ierr)
    call VecGetValues(SolVecir, nir*nVec, tempir, ybc, ierr)

    Vri = ybc((ni*nVec+1):nir*nVec)
...
!!-----------------------------------------------------------------------------------------------
    ...
    call VecGetValues(PetVecbg, nbg*nVec, tempbg, RZb, ierr)
    CALL MPI_REDUCE(RZb,Zb_g,nbg*nVec,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
...
!!!---------------------------------------------------------------------------------------------
    ...
    DO i = 1,maxiter
        ...
        !! STEP 12 : Scatter
        CALL MPI_BCAST(Pb_g,nbg*nVec,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
        call SetVecSeq(nbg*nVec,Pb_g,PetVecbg,ierr)
    ...
    !!!! Algorithm 3 - step 4 - 7
        ...
        CALL MPI_REDUCE(RZb,Qb_g,nbg*nVec,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    ...
    !!-----------------------------------------------------------------------------------------------
        ...
        CALL MPI_BCAST(rb_g,nbg*nVec,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
        call SetVecSeq(nbg*nVec,rb_g,PetVecbg,ierr)
        ...
        call VecGetValues(PetVecr, nr*nVec, tempr, FrS, ierr)
    ...
    !!-----------------------------------------------------------------------------------------------
        !! SUB-STEP 7 : Solve
        ybc(:) = 0.0d0
        ybc((ni*nVec+1):nir*nVec) = FrS   ! Force vector which needs to be used after the coarse solve
...
!!!---------------------------------------------------------------------------------------------
    ...
    CALL MPI_BCAST(Ub_g,nbg*nVec,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
    call SetVecSeq(nbg*nVec,Ub_g,PetVecbg,ierr)
    ...
    call VecGetValues(SolVeci, ni*nVec, tempi, Ui, ierr)
    call VecGetValues(PetVecb, nb*nVec, tempb, Ub, ierr)    !!!! In order to do the update at subdomain level
    ...
    if (outputFlag .eq. 1) then
        ...
        call create_Af_pckfddm3D(Zero,One,np,nb,Ui,Ub,nVec,Af)      !!!! Function to create full solution but Ui is only for a particular subdomain
        if (pid .eq. 0) then
          call create_Afb_pckfddm3D(Zero,One,nbg,Ub_g,nVec,Afb)      !!!! Temporary storing of boundary solution needed for final creation after reduce
        end if

        call construct_coln3D(pid,np,nb,npg,Af(1:(np-nb)*nVec,1),U_gi)         !!!! Putting all the interior nodes in order for display
        CALL MPI_REDUCE(U_gi,U_g,npg*nVec,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)      !!!! Summing up all the nodes from all subdoamins which
        ! results in sum of boundary nodes too which has to be replaced . Interior nodes when summed doesn't effetc each other

        if (pid .eq. 0) then
            call construct_coln_boundary3D(nbg,npg,Afb(:,1),U_g)        !!! Replacing the summed up boundary nodes to correct one
            ...
            if (pid .eq. 0) then
                call int2str(str1,nbcount,4)
                str1 = '../../data/vtkOutputs/U_mean_'// trim(str1) // '.dat'
                ! print *, str1
    ...
    call NewmarkBetaUpdate(pid, ni*nVec, nb*nVec, deltaT, beta_NB, gamma_NB, Ui, Ub, u0i, u0b, v0i, v0b, a0i, a0b, ierr)
...
!!----------------------------------------------------------------------------------------
!! All node numbers are already multiplied by nVec.
!!----------------------------------------------------------------------------------------
