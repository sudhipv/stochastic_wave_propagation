...
echo " 3 for 3D-Elasticity Stochastic"
...
sed -i -e 's#output=%x-%j.out#output=/scratch/a/asarkar/sudhipv/ssfem_wave/data/slurm/%j-%x.out#g' run_ddm.sh
fi

sed -i -e 's/time=0-00:10/time=01:30:00/g' run_ddm.sh
#sed -i -e 's/mem-per-cpu=7700M/mem-per-cpu=3700M/g' run_ddm.sh
#sed -i -e 's/tasks-per-node=32/tasks-per-node=32/g' run_ddm.sh
sed -i -e 's/nodes=1/nodes=4/g' run_ddm.sh
sed -i -e 's/-np 32/-np 160/g' run_ddm.sh
## Below d->dims, lc-> mesh density parameter, p->partitions
sed -i -e 's/job-name=DP_nRV_nNodes_nParts/job-name=wave_det_WB_160core_450k/g' run_ddm.sh
