...
then
    echo "MPI-Job: 2D-Acoustic Stochastic Two-level Selected"
...
echo "MPI-Job: Enter 1 for Cedar"
read input1
...
then
    ...
    cp run_ddm_cedar.sh ../run_ddm.sh
else
    echo "STOP: Wrong input"
    exit
fi
...
sed -i -e 's/time=0-00:10/time=02:35:00/g' run_ddm.sh
sed -i -e 's/mem-per-cpu=7700M/mem-per-cpu=3700M/g' run_ddm.sh
sed -i -e 's/tasks-per-node=32/tasks-per-node=48/g' run_ddm.sh
sed -i -e 's/nodes=1/nodes=17/g' run_ddm.sh
sed -i -e 's/-np 32/-np 800/g' run_ddm.sh
## Below d->dims, lc-> mesh density parameter, p->partitions
sed -i -e 's/job-name=DP_nRV_nNodes_nParts/job-name=wave_VB_7rv18k_PC_800core/g' run_ddm.sh
sed -i -e 's#output=%x-%j.out#output=/scratch/sudhipv/sudhipv/ssfem_wave/data/slurm/%j-%x.out#g' run_ddm.sh
