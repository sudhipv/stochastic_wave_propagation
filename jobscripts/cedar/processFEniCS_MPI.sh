...
echo "FEniCS: Enter 1 for Cedar"
read input1

if [ $input1 == 1 ]
then
    echo "FEniCS: Selected Cedar"
    cp fenics_activate_cedar.sh ../../wave/fenics_activate.sh
    cp fenics_activate_cedar.sh ../../static/fenics_activate.sh
else
...
if [ $input2 == 1 ]
then
    echo "** FEniCS: 2D-Acoustic Two-level Parallel Selected **"
...
sed -i -e 's/time=0-00:15/time=0-00:40/g' run_fenics.sh
sed -i -e 's/mem-per-cpu=7700M/mem-per-cpu=3700M/g' run_fenics.sh
sed -i -e 's/nodes=1/nodes=17/g' run_fenics.sh
sed -i -e 's/tasks-per-node=32/tasks-per-node=48/g' run_fenics.sh
sed -i -e 's/-n 32/-n 800/g' run_fenics.sh
sed -i -e 's#output=%j-3DPfenics.out#output=/scratch/sudhipv/sudhipv/ssfem_wave/data/slurm/fenics.out#g' run_fenics.sh
