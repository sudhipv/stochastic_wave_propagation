...
then
    ...
    cp fenics_activate_beluga.sh ../fenics_activate.sh
else
...
sed -i -e 's/time=0-00:15/time=0-04:00/g' run_fenics3D.sh
#sed -i -e 's/mem-per-cpu=7700M/mem-per-cpu=3700M/g' run_fenics3D.sh
#sed -i -e 's/--tasks-per-node=1/--tasks-per-node=40/g' run_fenics3D.sh
sed -i -e 's#output=%j-3Dfenics.out#output=/scratch/sudhipv/ssfem_fenics/data/slurm/3Dfenics-%j.out#g' run_fenics3D.sh
