%% Assemble Vector is used similar to the puffin Solver: Modified for specific use by Ajit
% Assemble a vector on a given mesh (points, edges, triangles) from a given variational formulation (pde)
% for more information refer PUFFIN-FEM Official DOC.

function b = AssembleVector(points, edges, triangles, pde, W, time, k)
...
for triangle = triangles
    ...
    for p = midpoints
        ...
        for i = 1:3
            ...
            integral = feval(pde, k, 0, v, w, [0;0], dv, dw, dx, 0, x, d, time, 2) / 3.0;
...
for edge = edges
    ...
    for p = gausspoints
        ...
        for i = 1:2
            ...
            integral = feval(pde, k, 0, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, 2) / 2.0;
            b(nodes(i)) = b(nodes(i)) + integral;
