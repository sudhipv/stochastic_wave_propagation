%% Assemble Matrix is used similar to the puffin Solver: Modified for specific use by Ajit
% Assemble a matrix for a given mesh data(points, edges, triangles) From a given variational formulation (pde)
% For more information refer PUFFIN-FEM Official DOC.

function A = AssembleMatrix(points, edges, triangles, pde, W, time, k)
...
for triangle = triangles
    ...
    nodes = triangle(1:3);
    coord = points(:,nodes);                            %% dbounds in my codes
    d = 0; % triangle(4) is wrong it should be triangle(5)=1;%% for boundary conditions
    ...
    for p = midpoints
        ...
        for i = 1:3
            ...
            for j = 1:3
                ...
                integral = feval(pde, k, u, v, w, du, dv, dw, dx, 0, x, d, time, 1) / 3.0;
    ...
    for p = gausspoints
        ...
        for i = 1:2
            ...
            for j = 1:2
                ...
                integral = feval(pde, k, u, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, 1) / 2.0;
                A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
