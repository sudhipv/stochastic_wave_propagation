...
% Direct / One Level Preconditioner - Fortran Decomposed Mesh
% Sudhi Sharma P V %
...
meshpath = sprintf('/Users/sudhipv/documents/ssfem_wave/data/meshData/');
...
node_ini = load(n_sub);  
% node_ini = unique(node_ini);

nN_s = length(node_ini);
...
un_interface = u_n(n_interface);
pn_interface = p_n(n_interface);
an_interface = a_n(n_interface);


un_interior = un_sub(1:nNi_s,1);
pn_interior = pn_sub(1:nNi_s,1);
an_interior = an_sub(1:nNi_s,1);
...
b_t_interior = bi + M_n_interior ;
b_t_interface = bg + M_n_interface ;
