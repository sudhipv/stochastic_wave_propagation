...
class MyExpression(Expression):
    ...
    def eval(self, values, x):
        ...
        multipliersY = [0.5678,0.2043,0.1265,0.1057,0.0905,0.0794,0.0634,0.0529]
        omegasY = [0.0320,0.08450,0.1390,0.1660,0.1940,0.2210,0.27610,0.33120]

        multipliers = [0.5653,0.3069,0.2089,0.1584,0.1265,0.1056,0.0906,0.0794]
        omegas = [0.0184,0.0330,0.04810,0.0630,0.07920,0.09480,0.110420,0.1260]


        # Mean and Standard Deviation of Underlaying Gaussian
        meang = 1.046      # exp(0+1/2*(sigma^2)), meanc in Fortran package
        sigma = 0.3      # mul=1.03 for mug=0,sigma=0.3 | mul=1.27 for mug=0,sigma=0.7
...
## Automation to n-RVs : ----------------------------------------------------------------
    ## KLE: obtain KLE terms
        ...
        for i in range(self.ndim):
            ...
            if (Xindex % 2) == 0:
                ##print("even")
                gg1 = multipliers[Xindex-1] * (sin(omegas[Xindex-1]*(x[0]-100.0)))
            else:
                ##print("odd")
                gg1 = multipliers[Xindex-1] * (cos(omegas[Xindex-1]*(x[0]-100.0)))

            Yindex = self.sIndex[i,1]
            if (Yindex % 2) == 0:
                ##print("even")
                gg2 = multipliersY[Yindex-1] * (sin(omegasY[Yindex-1]*(x[1]-57)))
            else:
                ##print("odd")
                gg2 = multipliersY[Yindex-1] * (cos(omegasY[Yindex-1]*(x[1]-57)))

            Zindex = self.sIndex[i,2]
            if (Zindex % 2) == 0:
                ##print("even")
                gg3 = multipliers[Zindex-1] * (sin(omegas[Zindex-1]*(x[2]-100.0)))
            else:
                ##print("odd")
                gg3 = multipliers[Zindex-1] * (cos(omegas[Zindex-1]*(x[2]-100.0)))
...
#### x[0] = 200, x[1] = 114, x[2] = 200 ##
    ...
    def top(x, on_boundary):
        return on_boundary and near(x[1],114.0)
...
#### Whether to apply BC to all terms or only mean term. ?
#### The bc.apply makes the system unsymmetric
#### zero_columns makes it symmetric
#### Using 1e7 effectively makes the application of BC as a penalty approach with penalty value of 1e7. However, whether this value is
#### large enough ?
    # if k ==0:
    for bc in bcs:
        bc.apply(A_g)
