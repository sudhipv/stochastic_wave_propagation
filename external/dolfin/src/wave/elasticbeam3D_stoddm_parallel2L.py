...
### Elastic wave propagation in a 3D cantilever beam
################################################################
...
rho = 1               ## Scaling using mu=rho=L=1
...
#velocity = sqrt((lambda_+2*mu)/rho) = 1.802
...
ui  = Expression(("0.0","5.0*exp(-0.01*(pow(x[0]-0.1,2)+pow(x[1]-0.1,2)+pow(x[2]-0.5,2)))","0.0"),degree=4)
# u0 = interpolate(ui, V)
