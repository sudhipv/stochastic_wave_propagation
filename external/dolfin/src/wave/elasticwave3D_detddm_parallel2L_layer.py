...
#### Application of Boundary condition is implemented in such a way so that condition number of the matrix is kept small by adding diagonal entries with large values.
################################################################
...
nu = 0.3
mu = E/(2.0*(1.0+nu))
lambda_ = (E*nu)/((1+nu)*(1-(2.0*nu))) ##(2.556*0.2778)/((1+0.2778)*(1-(2.0*0.2778)))=1.25


# print(mu)

# fcpath = "lamda.dat"
# np.savetxt(fcpath, lambda_)
# print(E.vector().array())
...
ui  = Expression(("0.0","5*exp(-0.01*(pow(x[0]-100.0,2)+pow(x[1]-90.0,2)+pow(x[2]-100.0,2)))","0.0"),degree=4)
# u0 = interpolate(ui, V)
...
#m = Form((rho * Constant(0.0) *  dot(u,v)*dx))
l = Form(dot(f, v)*dx )
