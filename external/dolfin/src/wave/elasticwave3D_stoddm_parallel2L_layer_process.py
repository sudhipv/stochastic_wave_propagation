...
class MyExpression(Expression):
    ...
    def eval(self, values, x):
        ...
        sigma = 0.1      # mul=1.03 for mug=0,sigma=0.3 | mul=1.27 for mug=0,sigma=0.7
...
## Automation to n-RVs : ----------------------------------------------------------------
    ## KLE: obtain KLE terms
        ...
        # print("values from process are", values[0])
        values[0] = meang * newY
...
def stoVartional(params, u, v, epsilon, f):
    ...
    # if (ip == 0):
    #     print("layered E values inside stochastic part are" , E.vector().array())

    Es = E * MyExpression(params, degree=0)

    # if (ip == 0):
    #     print("layered Es values inside stochastic part are" , Es)
...
E.vector()[:] = np.choose(help, E_lvalues)
