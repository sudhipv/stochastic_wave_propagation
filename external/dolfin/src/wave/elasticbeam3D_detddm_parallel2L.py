...
#### Deterministic Test Case : Forcing boudary ##############################
...
#######################################################################
    ...
    L_g = assemble(Constant(0.)*(dot(f, v)*dx + dot(Tr, v)*ds))
...
#################################################
    ...
    bc_L = DirichletBC(V, Constant((0, 0, 0)), clamped_boundary)
    ...
    # bc_L = DirichletBC(V, Constant((0.0, 0.0, 0.0)), left)
    # bc_R = DirichletBC(V, Constant((0.0, 0, 0.0)), right)
    # bc_T = DirichletBC(V, Constant((0.0, 0.0, 0.0)), top)
...
## For beam with beam(0.2,0.2,1) - Ajits elasticity
rho = 1               ## Scaling usng mu=rho=L=1
delta = 0.2           ## Width(0.2)/Length(1)
g = 0.4*delta**2      ## FenicsTutorial suggestion 0.4 for beam(0.2,0.2,1)
nu = 0.2778           ## Calculated using mu=1,lambda=1.25
E = 2.5556            ## Calculated using mu=1,beta=1.25
mu = 1.0                  ##(1*nu)/((1+nu)*(1-(2.0*nu))) = 1
lambda_ = 1.25 ##(2.556*0.2778)/((1+0.2778)*(1-(2.0*0.2778)))=1.25
...
#Linearly varying traction from 0-1 until t/5 and then removed
# p0 = 1.
# cutoff_Tc = T/5
# # Define the loading as an expression depending on t
# Tr = Expression(("0", "t <= tc ? p0*t/tc : 0", "0"), t=0, tc=cutoff_Tc, p0=p0, degree=0)

# Tr = Constant((0, 1, 0))

### Gaussian Pulse Initail Condition
# ui  = Expression(("1*exp(-100*(pow(x[0]-0.7,2)+pow(x[1]-0.7,2)))"),degree=1)
# ui  = Expression(("1.0*exp(-1000.*(pow(x[2]-0.5,2)+pow(x[1]-0.2,2)+ pow(x[0]-0.1,2)))","1.0*exp(-1000.*(pow(x[2]-0.5,2)+pow(x[1]-0.2,2)+ pow(x[0]-0.1,2)))", "1.0*exp(-1000.*(pow(x[2]-0.5,2)+pow(x[1]-0.2,2)+ pow(x[0]-0.1,2)))"), degree=2)

# u0 = interpolate(ui, V)

ui = Expression(("0.0","0.0","0.0"), degree=1)
vi = Expression(("0.0","0.0","0.0"), degree=1)
ai = Expression(("0.0","0.0","0.0"), degree=1)
...
if (ip == 0):
    np.savetxt("./u0_0.dat",u0.vector().array())
else:
    np.savetxt("./u0_1.dat",u0.vector().array())
