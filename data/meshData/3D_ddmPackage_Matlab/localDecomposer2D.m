...
trianglesNotShrd = dlmread('../trianglesNotShrd.dat', '');
trianglesShrd = dlmread('../trianglesShrd.dat', '');
...
triangles_on_boundary = dlmread('../triangles_on_boundary.dat', '');
for i = 1:nb_sus
...
for k = 1:ndom
   ...
   test1 = unique(edges(:,5));
    if k ~= test1
       ...
       FSD = [FSD; k];
       aaa = [];
       %floatingEdge  = ['../edges000',num2str(domainNumber)];
       floatingEdge  = ['../edges', pad(num2str(domainNumber),nzs,'left','0')];
    ...
    for i = 1:ne
        j = edges(i:i,5:5);             
        if j == k
            %tempEdg  = ['../edges000',num2str(j)];
            tempEdg  = ['../edges', pad(num2str(j),nzs,'left','0')];
    ...
    for l = 1:nt
        ...
        if m == k
            %nameT  = ['../triangles000',num2str(m)];
            nameT  = ['../triangles', pad(num2str(m),nzs,'left','0')];
...
for i = 1:ndom
   ...
   for j = 1:nbi(i)
    ...
    tempTriangles2 =  ['../triangles', pad(num2str(i),nzs,'left','0')];
