...
for i = 1:ndom     
    ...
    tempNodes2  = ['../nodes', pad(num2str(i),npad,'left','0')];
    nodeTemp1 = dlmread([tempNodes2 '.dat']);
    nN = length(nodeTemp1);
    
    %tempBNodes2  = ['../bnodes00',num2str(i)];
    tempBNodes2  = ['../bnodes', pad(num2str(i),npad,'left','0')];
    bnodeTemp1 = dlmread([tempBNodes2 '.dat']);
    nB = length(bnodeTemp1);
    
    %tempRNodes2  = ['../rnodes00',num2str(i)];
    tempRNodes2  = ['../rnodes', pad(num2str(i),npad,'left','0')];
    rnodeTemp1 = dlmread([tempRNodes2 '.dat']);
    nR = length(rnodeTemp1);
    
    %tempCNodes2  = ['../cnodes00',num2str(i)];
    tempCNodes2  = ['../cnodes', pad(num2str(i),npad,'left','0')];
    cnodeTemp1 = dlmread([tempCNodes2 '.dat']);
    nC = length(cnodeTemp1);
    
    Bnodes = [rnodeTemp1 ; cnodeTemp1];   %% B = [R then C]
    %bnodes2  = ['../nbnodes00',num2str(i)];       % ['NBnodes00',num2str(i)];
    bnodes2  = ['../nbnodes', pad(num2str(i),npad,'left','0')];
    ...
    %tempPoints  = ['../Nnodes00',num2str(i)];   %% N = [I, B] = [I, R, C]
    tempPoints  = ['../Nnodes', pad(num2str(i),npad,'left','0')];
    dlmwrite([tempPoints '.dat'], nodeTemp1, '\t');
    
    for j=1:nN
        pointsTemp2 = points(nodeTemp1(j),:);
        %nameP  = ['../rpoints00',num2str(i)];      %['npoints00',num2str(i)];
        nameP  = ['../rpoints', pad(num2str(i),npad,'left','0')];
...
%     end
    ...
    newNameE  = ['../redges', pad(num2str(i),npad,'left','0')];
    dlmwrite([newNameE '.dat'], NewEdges);  
    
    %%% Triangles:
    %tempReT2  = ['../triangles00',num2str(i)];
    
    res = ismember(i, flotDom);
    if res == 1
        tempReT2  = ['../triangles', pad(num2str(i),npad,'left','0')];
       ...
       for j = 1:nT  %nEdg
        ...
        newNameT  = ['../rtriangles', pad(num2str(i),npad,'left','0')];
        dlmwrite([newNameT '.dat'], NewTriangles); 
    end
    
    %%% Tetrahedrons
    %tempReH2  = ['../mtetrahedrans00',num2str(i)];
    tempReH2  = ['../mtetrahedrans', pad(num2str(i),npad,'left','0')];
   ...
   for j = 1:nH  %nEdg
    ...
    newNameH  = ['../rtetrahedrons', pad(num2str(i),npad,'left','0')];
    dlmwrite([newNameH '.dat'], Newtetrahedrons);    
    
    %newNameH2  = ['../rmtetrahedrons00',num2str(i)];
    newNameH2  = ['../rmtetrahedrons', pad(num2str(i),npad,'left','0')];
...
for i = 1:ndom           % rearranged nBnodes(r, c) %only for two level
    %tempBN1  = ['../nbnodes00',num2str(i)];% ['NBnodes00',num2str(i)];
    tempBN1  = ['../nbnodes', pad(num2str(i),npad,'left','0')];
   ...
   for j = 1:nB  %nPar
    ...
    newNameLB  = ['../rbnodes', pad(num2str(i),npad,'left','0')];
